...
param(
    ...
    [Parameter()]
    [string]$ProjectPath = (Join-Path $RepoRoot 'StorageWatchServer\StorageWatchServer.csproj'),
...
if ([string]::IsNullOrWhiteSpace($PublishDir)) {
    $PublishDir = Join-Path $OutputRoot 'publish/server'
}

if ([string]::IsNullOrWhiteSpace($PackageOutputDir)) {
    $PackageOutputDir = Join-Path $OutputRoot 'packages/server'
}

if ([string]::IsNullOrWhiteSpace($PackageFileName)) {
    $PackageFileName = "StorageWatchServer.$Version.zip"
}

if ([string]::IsNullOrWhiteSpace($PayloadComponentDir)) {
    $PayloadComponentDir = Join-Path $PayloadRoot 'Server'
}
...
if (-not $PSCmdlet.ShouldProcess("Action description", "Action name")) {
    ...
    if ($StageToPayload) {
        Write-Host "[WhatIf] Would stage Server payload to '$resolvedPayloadComponentDir'."
    }
    return
}
